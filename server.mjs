...
import { copilotAuth } from './scripts/copilot-auth.mjs';
...
function seed() {
  ...
  p.run('github-copilot','GitHub Copilot','https://api.githubcopilot.com','gpt-4o',0.2,60,4096,0,'');
}
seed();
// 老库升级：seed() 对已有数据直接跳过，这里单独补齐 github-copilot 预置厂商
if (!db.prepare("SELECT 1 FROM providers WHERE id='github-copilot'").get()) db.prepare('INSERT INTO providers VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run('github-copilot','GitHub Copilot','https://api.githubcopilot.com','gpt-4o',0.2,60,4096,0,'');
...
const copilot = copilotAuth({ save: saveProviderKey, read: readProviderKey });
...
async function handleApi(req,res,url){
  ...
  try{
    ...
    if(parts[1]==='providers'&&parts[2]==='github-copilot'&&parts[3]==='auth'){
      if(req.method==='GET')return send(res,200,await copilot.status());
      if(req.method==='DELETE'){await copilot.logout();audit('provider','github-copilot','退出登录');return send(res,200,{ok:true});}
      if(req.method==='POST'){const d=await readBody(req);
        if(d.action==='start'){audit('provider','github-copilot','发起设备码登录');return send(res,200,await copilot.start());}
        if(d.action==='poll'){const r=await copilot.poll(d.deviceCode);
          if(r.status==='success'){db.prepare('UPDATE providers SET key_mask=? WHERE id=?').run(r.mask||'','github-copilot');audit('provider','github-copilot','Copilot 设备码授权成功');}
          else if(r.status==='error'||r.status==='expired')audit('provider','github-copilot','授权未完成',r.message||r.status);
          return send(res,200,r);}
        return send(res,400,{error:'未知的授权动作'});
      }
    }
    ...
    if(req.method==='POST'&&parts[1]==='providers'&&parts[3]==='test'){const p=db.prepare('SELECT * FROM providers WHERE id=?').get(parts[2]);if(!p)return send(res,400,{error:'模型厂商不存在'});let headers;
      if(/api\.githubcopilot\.com/.test(p.base_url)){try{headers=(await copilot.getBearer()).headers}catch(e){return send(res,400,{error:e.message})}}
      else{const key=await readProviderKey(parts[2]);if(!key)return send(res,400,{error:'请先保存 API Key'});headers={Authorization:`Bearer ${key}`}}
      const started=Date.now(),response=await fetch(`${p.base_url.replace(/\/$/,'')}/models`,{headers,signal:AbortSignal.timeout(p.timeout*1000)});if(!response.ok)return send(res,502,{error:`连接失败：HTTP ${response.status}`});return send(res,200,{ok:true,latency:Date.now()-started});}
    if(req.method==='POST'&&parts[1]==='ai'&&parts[2]==='analyze'){const d=await readBody(req),p=db.prepare('SELECT * FROM providers WHERE id=? AND enabled=1').get(d.providerId);if(!p)return send(res,400,{error:'请选择并启用一个模型配置'});let authHeaders;if(/api\.githubcopilot\.com/.test(p.base_url)){try{authHeaders=(await copilot.getBearer()).headers}catch(e){return send(res,400,{error:e.message})}}else{const key=await readProviderKey(p.id);if(!key)return send(res,400,{error:'模型 API Key 未配置'});authHeaders={Authorization:`Bearer ${key}`}}const response=await fetch(`${p.base_url.replace(/\/$/,'')}/chat/completions`,{method:'POST',headers:{'Content-Type':'application/json',...authHeaders},body:json({model:p.model,temperature:p.temperature,max_tokens:p.max_tokens,messages:[{role:'system',content:'你是测试分析助手。仅返回合法 JSON，包含 summary、risk、suggestions、missingFields。'},{role:'user',content:`${d.taskType}\n${d.text}` }]}),signal:AbortSignal.timeout(p.timeout*1000)});if(!response.ok)return send(res,502,{error:`模型调用失败：HTTP ${response.status}`});const result=await response.json(),content=result.choices?.[0]?.message?.content||'{}';return send(res,200,{provider:p.name,model:p.model,result:parse(content.replace(/^```json\s*|\s*```$/g,''),{summary:content})});}
...
server.listen(port,host,()=>console.log(`AI测试工作台 已启动：http://127.0.0.1:${port}（局域网 http://<本机IP>:${port}）`));
